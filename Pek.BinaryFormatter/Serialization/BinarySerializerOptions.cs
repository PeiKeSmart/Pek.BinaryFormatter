...
public sealed partial class BinarySerializerOptions
{
    ...
    public BinaryIgnoreCondition DefaultIgnoreCondition
    {
        ...
        set
        {
            ...
            {
                throw new ArgumentException(PekLanguage?.Translate("The value cannot be 'BinaryIgnoreCondition.Always'."));
            }

            if (value != BinaryIgnoreCondition.Never && _ignoreNullValues)
            {
                throw new InvalidOperationException(PekLanguage?.Translate("'IgnoreNullValues' and 'DefaultIgnoreCondition' cannot both be set to non-default values."));
            }
